...
namespace Cosmos
{
	...
	Shared<TextureCubemap> TextureCubemap::Create(Shared<VKDevice> device, std::array<std::string, 6> paths, MSAA msaa)
	{
		return CreateShared<VKTextureCubemap>(device, paths, (VkSampleCountFlagBits)msaa);
	}
}